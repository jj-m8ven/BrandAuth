...
const statusStyles: Record<AuthorizationStatus, string> = {
  authorized: 'bg-violet-100 text-violet-700',
...
export function AuthorizationBadge({ status }: { status: AuthorizationStatus }) {
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-medium capitalize ${statusStyles[status]}`}
    >
